...
const MouseGlowOrb = ({ mouseX, mouseY, isInFooter }: { mouseX: number; mouseY: number; isInFooter: boolean }) => {
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        scale: isInFooter ? 1 : 0.5,
      }}
      transition={{ duration: 0.3 }}
    >
      {/* Main glow orb */}
      <div 
        className="absolute -translate-x-1/2 -translate-y-1/2 h-40 w-40 rounded-full"
        style={{
      ...
      {/* Inner bright core */}
      <motion.div 
        className="absolute -translate-x-1/2 -translate-y-1/2 h-16 w-16 rounded-full"
        style={{
      ...
      {/* Outer glow ring */}
      <motion.div 
        className="absolute -translate-x-1/2 -translate-y-1/2 h-24 w-24 rounded-full border border-primary/20"
        animate={{
          scale: [1, 1.3, 1],
          opacity: [0.3, 0.1, 0.3],
        }}
...
const Footer = () => {
  ...
  const handleMouseEnter = useCallback(() => setIsInFooter(true), []);
  const handleMouseLeave = useCallback(() => setIsInFooter(false), []);
  ...
  return (
    ...
    >
      ...
      <MouseGlowOrb mouseX={mousePos.x} mouseY={mousePos.y} isInFooter={isInFooter} />
